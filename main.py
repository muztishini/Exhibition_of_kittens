...
from schemas import BreedSchema, KittenSchema
...
@app.get("/api/kittens", response_model=List[KittenSchema])
async def get_kittens():
	kittens = db.query(Kitten).all()
	return kittens

@app.get("/api/kittens/{id}", response_model=KittenSchema)
